...
public class MainActivity extends AppCompatActivity implements View.OnClickListener{//初始化按钮
    ...
    @Override
    protected void onCreate(Bundle savedInstanceState) {//在按钮上面添加按钮事件
        super.onCreate(savedInstanceState);
        setContentView(R.layout.calculator);//设置布局
    ...
    @Override
    public void onClick(View v) {//按钮事件判断
        String str = text.getText().toString();//每次都得到当前的文本

        switch(v.getId()){//得到按钮点击事件的对应的按钮
            case R.id.btn_del:
                if(str.length()>1){
        ...
        if(str.length() > 10){
            str=str.substring(0,str.length()-1);//用于判断数值长度，结果保留10位有效数字
        }
    }
    ...
    public static StringBuffer toPostfix(String infix){
        ...
        while(i<infix.length()){
            ...
            switch(ch){
                case '+':
                case '-':
                    while(!stack.isEmpty()&&!stack.peek().equals("(")) //如果栈不为空
